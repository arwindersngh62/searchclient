...
class State:
    ...
    def apply_action(self, joint_action: '[Action, ...]') -> 'State':
        ...
        for agent, action in enumerate(joint_action):
            if action.type is ActionType.NoOp:
                ...
                copy_agent_rows[agent] += action.agent_row_delta
                copy_agent_cols[agent] += action.agent_col_delta
                boxRow = copy_agent_rows[agent]
                boxCol = copy_agent_cols[agent]
                box = copy_boxes[boxRow][boxCol]
                copy_boxes[boxRow][boxCol] = '\0'
                boxRow += action.box_row_delta
                boxCol += action.box_col_delta
                copy_boxes[boxRow][boxCol] = box
                

            elif action.type is ActionType.Pull:
                boxRow = copy_agent_rows[agent] - action.box_row_delta
                boxCol = copy_agent_cols[agent] - action.box_col_delta
                box = copy_boxes[boxRow][boxCol]
                copy_boxes[boxRow][boxCol] = '\0'
    ...
    def is_applicable(self, agent: 'int', action: 'Action') -> 'bool':
        ...
        if action.type is ActionType.NoOp:
            ...
            box_current_row = agent_row + action.agent_row_delta
            box_current_col = agent_col + action.agent_col_delta
            box_char = self.boxes[box_current_row][box_current_col]
            #print(len(box_char),box_char)
            if len(box_char)==0:
                return False
            else:
                box_color_int = ord(box_char) - 65
                if box_color_int < 0:
                    return False
            if self.box_colors[box_color_int] == agent_color:
                box_destination_row = box_current_row + action.box_row_delta
                box_destination_col = box_current_col + action.box_col_delta
                return self.is_free(box_destination_row,box_destination_col)

        elif action.type is ActionType.Pull:
            box_current_row = agent_row - action.box_row_delta
            box_current_col = agent_col - action.box_col_delta
            box_char = self.boxes[box_current_row][box_current_col] 
            if len(box_char) == 0:
                return False
            else:
    ...
    def is_conflicting(self, joint_action: '[Action, ...]') -> 'bool':
        ...
        for agent in range(num_agents):
            ...
            if action.type is ActionType.NoOp:
                ...
                destination_rows[agent] = agent_row + action.agent_row_delta
                destination_cols[agent] = agent_col + action.agent_col_delta
                box_current_row = destination_rows[agent]
                box_current_col = destination_cols[agent]
                box_rows[agent] = box_current_row + action.box_row_delta
                box_cols[agent] = box_current_col + action.box_col_delta
        ...
        for a1 in range(num_agents):
            ...
            for a2 in range(a1 + 1, num_agents):
                ...
                # Box and agent conflict?
                if box_rows[a1] == box_rows[a2] and box_cols[a1] == box_cols[a2]:
                    return True

                if box_rows[a1] == destination_rows[a2] and box_cols[a1] == destination_cols[a2]:
                    return True
